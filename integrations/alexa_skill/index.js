    ...
    async handle(handlerInput) {
        return new Promise((resolve, reject) => {
            fetchData(endpointUrl, (error, jsonData) => {
                ...
                const lastReadingTime = jsonData.last_reading;
                const rawDifference = parseInt(jsonData.value_difference);
                const isHigher = rawDifference >= 1;
                const diff = Math.abs(rawDifference);
                
                const sayCurrentReading = (glucoseValue) ? `Your sugar level is ${glucoseValue}` : "I'm having trouble reading your current glucose level. Please try again later.";
                const sayTrend = (glucoseTrend) ? `and ${glucoseTrend}` : '';
                const sayDifference = (rawDifference) ? `This is ${diff} ${isHigher ? 'higher' : 'lower'} than your previous reading.` : '';
                const sayTimeAgo = (lastReadingTime) ? `Last checked ${lastReadingTime}.` : '';
                
                const sayLatestGlucoseReading = `${sayCurrentReading} ${sayTrend}. ${sayDifference} ${sayTimeAgo}`;
                
                if (error) {
                    resolve(handlerInput.responseBuilder.speak(alexaSays.error).getResponse());
                } else {
                    resolve(handlerInput.responseBuilder.speak(sayLatestGlucoseReading).getResponse());
                }
            });
        });
    }
